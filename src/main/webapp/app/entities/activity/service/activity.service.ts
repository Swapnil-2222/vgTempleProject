...
type RestOf<T extends IActivity | NewActivity> = Omit<T, 'date' | 'lastModified' | 'freeField7' | 'freeField8'> & {
  date?: string | null;
...
export class ActivityService {
  ...
  protected convertDateFromClient<T extends IActivity | NewActivity | PartialUpdateActivity>(activity: T): RestOf<T> {
    return {
      ...activity,
      date: activity.date?.toJSON() ?? null,
      lastModified: activity.lastModified?.toJSON() ?? null,
      freeField7: activity.freeField7?.toJSON() ?? null,
      freeField8: activity.freeField8?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restActivity: RestActivity): IActivity {
    return {
      ...restActivity,
      date: restActivity.date ? dayjs(restActivity.date) : undefined,
      lastModified: restActivity.lastModified ? dayjs(restActivity.lastModified) : undefined,
      freeField7: restActivity.freeField7 ? dayjs(restActivity.freeField7) : undefined,
      freeField8: restActivity.freeField8 ? dayjs(restActivity.freeField8) : undefined,
    };
  }
